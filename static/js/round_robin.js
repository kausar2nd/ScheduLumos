...
function validateInputs() {
    ...
    if (quantum === '' || parseInt(quantum) <= 0) isValid = false;
...
document.getElementById('run-algo').addEventListener('click', function () {
    ...
    const formData = `arrival-time=${arrivalTimes}&burst-time=${burstTimes}&time-quantum=${quantum}`;
    fetchAndDisplayResults('/round-robin', formData);
});
